    ...
    if (this.childList.length === 0) {
      ...
      for (let theta = thetaMin; theta < thetaMax; theta += dTheta) {
        for (let phi = phiMin; phi < phiMax; phi += dPhi) {
          const p1 = Utils.rThetaPhiToXYZ(r, theta, phi);
          const p2 = Utils.rThetaPhiToXYZ(r, theta + dTheta, phi);
          const p3 = Utils.rThetaPhiToXYZ(r, theta + dTheta, phi + dPhi);
          const p4 = Utils.rThetaPhiToXYZ(r, theta, phi + dPhi);
