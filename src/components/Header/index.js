...
class Header extends React.Component {
  ...
  scenarioSelectionHandler(id) {
    ...
    if (scenarios) {
      const selectedScenario = scenarios.filter(s => s.ScenarioID === id)[0];
  ...
  createBusCount() {
    if (this.props.scenarioData && this.props.scenarioData.data) {
      return (
        <div className="header-items header-right-content">
  ...
  createScenarioDetails() {
    return this.props.activeScenario ? (
      <b>
        {" "}
        Scenario ID: {this.props.activeScenario.ScenarioID} | Name:{" "}
        {this.props.activeScenario.Name}{" "}
      </b>
    ) : null;
  }
